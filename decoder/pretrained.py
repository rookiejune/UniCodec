...
class Unicodec(nn.Module):
    ...
    def forward(self, audio_input: torch.Tensor, domain, **kwargs: Any) -> torch.Tensor:
        """
        ...
        """
        features, _, _ = self.feature_extractor.infer(audio_input, domain, **kwargs)  # 0818
    ...
    # 0818
    @torch.inference_mode()
    def encode_infer(self, audio_input: torch.Tensor, domain, **kwargs: Any) -> torch.Tensor:
        features, discrete_codes, _ = self.feature_extractor.infer(audio_input, domain, **kwargs)
        return features,discrete_codes
