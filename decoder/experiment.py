...
class VocosExp(pl.LightningModule):
    ...
    def __init__(
        ...
        self.ema = EMA(models=[self.feature_extractor,self.backbone,self.head], decay = 0.99)
    ...
    def forward(self, audio_input, audio_domain, **kwargs):
        ##contrastive
        codes, commit_loss, cnn_feat, mask_indices, features = self.feature_extractor(audio_input, audio_domain, **kwargs)
    ...
    def training_step(self, batch, batch_idx, optimizer_idx, **kwargs):
        ...
        if optimizer_idx == 0 and self.train_discriminator:
            with torch.no_grad():
                audio_hat, _ = self(audio_input, audio_domain, **kwargs)
        ...
        # train generator
        if optimizer_idx == 1:
            audio_hat, commit_loss = self(audio_input, audio_domain, **kwargs)
            ...
            if self.use_ema:
                # breakpoint()
                self.ema.update([self.feature_extractor,self.backbone,self.head])
    ...
    def validation_step(self, batch, batch_idx, **kwargs):
        audio_input, audio_domain = batch
        if self.use_ema:
            self.ema.store([self.feature_extractor,self.backbone,self.head])
            self.ema.apply_shadow([self.feature_extractor,self.backbone,self.head])
        ...
        if self.use_ema:
            self.ema.restore([self.feature_extractor,self.backbone,self.head])
