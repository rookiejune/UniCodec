...
from .loss import DiscriminatorLoss, GeneratorLoss, FeatureMatchingLoss, MelSpecReconstructionLoss, DACGANLoss, Wav2vecCriterion
...
class VocosExp(pl.LightningModule):
    ...
    def __init__(
        ...
        self.wav2vec_contrastive_loss = Wav2vecCriterion("audio_pretaining")
    ...
    def forward(self, audio_input, audio_domain, **kwargs):
        codes, commit_loss, cnn_feat, mask_indices, quantized = self.feature_extractor(audio_input, audio_domain, **kwargs)
        # print('1111', self.feature_extractor.state_dict()['encodec.decoder.model.3.convtr.convtr.weight_g'])
        x = self.backbone(quantized, **kwargs)
        audio_output = self.head(x)
        return audio_output, commit_loss, cnn_feat, mask_indices, quantized

    def training_step(self, batch, batch_idx, optimizer_idx, **kwargs):
        # import pdb; pdb.set_trace()
        audio_input, audio_domain = batch

        # train discriminator
        if optimizer_idx == 0 and self.train_discriminator:
            with torch.no_grad():
                audio_hat, _, _, mask_indices, _ = self(audio_input, audio_domain, **kwargs)
        ...
        # train generator
        if optimizer_idx == 1:
            audio_hat, commit_loss, cnn_feat, mask_indices, quantized = self(audio_input,audio_domain, **kwargs)
            ...
            if self.train_discriminator:
                ...
                loss_contrastive = self.wav2vec_contrastive_loss(cnn_feat, mask_indices, quantized)
                ...
                self.log("generator/loss_contrastive", loss_contrastive)
            ...
            loss = (
                ...
                + loss_dac_1
                + loss_dac_2
                + self.hparams.ctr_loss_coeff * loss_contrastive
            )
    ...
    def validation_step(self, batch, batch_idx, **kwargs):
        audio_input, audio_domain = batch
        audio_hat, commit_loss, _, _, _ = self(audio_input, audio_domain, **kwargs)
